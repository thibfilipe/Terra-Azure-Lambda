...
# Create HDI Subnet
resource "azurerm_subnet" "Subnet-LambdaHDI" {
  name                 = "${var.HDISubnetName}"
  resource_group_name  = "${azurerm_resource_group.RSG-Lambda.name}"
  virtual_network_name = "${azurerm_virtual_network.VNet-LambdaVM.name}"
  address_prefix       = "${var.HDIAddressPrefix}"
}
...
DEPLOY
  ...
  parameters {
    "existingVnetName" = "${azurerm_virtual_network.VNet-LambdaVM.name}"
    "existingSubNetName" = "${azurerm_subnet.Subnet-LambdaHDI.name}"
...
# "[concat('Microsoft.Network/virtualNetworks/',parameters('existingSubnetName'))]"